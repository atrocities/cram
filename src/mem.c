...
int initMemOld() {
	...
	if (HRAM == null)
		return -1;
	IOBUF = (byte*) malloc(sizeof(byte) * 0x80);
	if (IOBUF == null)
		return -1;
	VRAM = (byte*) malloc(sizeof(byte) * 0x2000);
	if (VRAM == null)
		return -1;
	for (i = 0; i < 0x2000; i++) {
		VRAM[i] = 0;
	...
	if (ERAM == null)
		return -1;
	WRAM1 = (byte*) malloc(sizeof(byte) * 0x2000);
	if (WRAM1 == null)
		return -1;
	for (i = 0; i < 0x2000; i++) 
		WRAM1[i] = 0;
	WRAM2 = (byte*) malloc(sizeof(byte) * 0x1000);
	if (WRAM2 == null)
		return -1;
	for (i = 0; i < 0x1000; i++) 
		WRAM2[i] = 0;
	OAM = (byte*) malloc(sizeof(byte) * 0x100);
	if (OAM == null)
		return -1;
	for (i = 0; i < 0x100; i++)
		OAM[i] = 0;
...
byte readbyte(word address) {
	return fastread(address);
}

word readword(word address) {
	//printf("reading word: %d\n", address);
	return (fastread(address) | (fastread(address + 1) << 8));
}

int writebyte(word address, byte val) {
	return fastwrite(address, val);
}

int writeword(word address, word val) {
	fastwrite(address, (byte) (val & 0xFF));
	return fastwrite(address + 1, (byte) (val >> 8));
}
...
void doDma(byte addrl) {
	...
	OAM = MEM + address;
	//for (i = 0; i < 0x9F; i++) {
	//	OAM[i] = fastread(address + i);
	//}
	//DMADELAY = 140;
}

void oldDoDma(byte addrl) {
	word address = addrl << 8;
...
byte fastread(word address) {
	//if (address >= 0xFE00 && address <= 0xFE9F) {
	//	address = address & 0xFF + dmaOffset;
	//}
        if (address <= 0x3FFF)
              return ROM[address];
        else if (address <= 0x7FFF) {
...
word fastwrite(word address, byte val) {
        if (address <= 0x7FFF) {
                if (mreader == null) {
                        return 0;
                } else {
